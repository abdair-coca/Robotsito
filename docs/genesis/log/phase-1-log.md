...
## 2026-06-29
**Tipo:** fix
**Componente:** firmware/audio (parlante ESP32)
**Descripción:** Puesto a punto el parlante del ESP32 (DAC interno + PAM8403) para que el TTS suene fuerte y limpio. Tres problemas resueltos en cadena: (1) **Calidad/volumen** — la voz sonaba floja y con grano vs la música. Causa: la cadena de filtros TTS (`config.py:TTS_FFMPEG_FILTERS`) no normalizaba dinámica, así que los pasajes flojos subutilizaban los 8 bits. Fix: `speechnorm` (normalizador de voz) + `acompressor` (ratio 4:1, makeup +4dB) + `alimiter` 0.98 + `TTS_VOLUME` 1.0. Medido en voz real: nivel medio −21.2 → −12.2 dB (+9 dB, ~1.8× percibido) sin clip. (2) **No se oía nada** — el firmware tenía el DAC en `Pin(25)` pero el parlante estaba cableado a **GPIO26** (verificado con `TestMusic.py`, que suena fuerte a solo 30% de amplitud en Pin26). Fix: firmware `DAC(Pin(26))`. (3) **Audio entrecortado ("motor arrancando")** al integrar con el resto — el hilo OLED hace `show()` por I2C que **bloquea el bus ~25ms** y le roba CPU al busy-wait de audio (8 kHz); a `OLED_FPS_MS_AUDIO=220` daba un hueco cada 220ms ≈ stutter de 4.5 Hz. `TestMusic.py` no lo tenía (sin OLED, un solo hilo). Fix en `firmware/Esp32/main.py` (`hilo_oled`): durante `reproduciendo`, dibujar UN frame al entrar a HABLANDO y luego **congelar el OLED** (cero I2C) hasta que termine el audio. Validado: audio liso en hardware integrado. También documentado el cableado para conservar calidad (rail de 5V limpio para el amp, cap bulk 470-1000µF, tierra en estrella, señal DAC lejos de PWM de servos/motores).
**Resultado:** Bob habla por su propio parlante con volumen fuerte, calidad limpia y **sin cortes**, con servos/OLED/motores integrados. Speaker ya no depende de la laptop. Cambios en `config.py` (gitignored, local) y `firmware/Esp32/main.py`. Runner de prueba aislado: `robot_bob/tests/test_3_voice_speaker.py`.
**Próximo paso:** Validar en sesión larga moviendo servos/motores mientras Bob habla (confirmar que no hay brownout). Salto futuro de fidelidad: migrar a I2S (ADR-001). Prioridad de roadmap sigue siendo **P5 (Juegos)**.

## 2026-06-29
**Tipo:** build
**Componente:** voz/música (P6)
**Descripción:** Implementado P6 (Música) — control de Spotify por voz. Nuevo módulo `robot_bob/music.py`, patrón intent+acción (como `reminders.py`): `parse_music_command(texto)` detecta play/pausa/siguiente/anterior/subir-bajar volumen y "pon/reproduce/quiero escuchar X" extrayendo el tema; `ejecutar(intent)` llama a la **Spotify Web API** vía `spotipy` (OAuth Authorization Code, scopes `user-modify/read-playback-state`, token cacheado en `.spotify_cache` gitignored) y devuelve la frase que dice Bob. La música suena por el dispositivo **Spotify Connect activo** (laptop con Spotify abierto), NO por el parlante del robot; requiere cuenta **premium**. Wireado en `voice_pipeline._run_conversation` (bloque antes de los comandos de salida; `continue` para no pasar al LLM). Credenciales en `.env` (`SPOTIFY_CLIENT_ID/SECRET/REDIRECT_URI`), leídas en `config.py`; `MUSICA_ENABLED` se auto-activa si hay client id+secret. Defensivo: sin credenciales/spotipy/dispositivo/premium no rompe la charla (parser devuelve None o frase amable). Dependencia nueva `spotipy>=2.23` en requirements. Test `tests/test_6_musica.py` (parser + negativos + degradación sin credenciales) — TODO OK.
**Resultado:** Bob controla la música por voz ("pon despacito", "siguiente", "sube el volumen", "pausa"). Falta que Abdair ponga las credenciales reales de Spotify en `.env` y autorice una vez (se abre el navegador). ROADMAP: P6 marcado hecho (control). Entretenimiento LLM (chistes/historias) ya lo cubre la charla normal.
**Próximo paso:** Abdair crea la app en developer.spotify.com, pone credenciales en `.env`, prueba en vivo con Spotify premium abierto. Luego **P5 (Juegos)**.
