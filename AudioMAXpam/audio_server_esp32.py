...
import network, usocket, utime, struct

# ── Configuración ─────────────────────────────────────────────────
SSID     = 'Familia Coca Carlo -5'      # Cambia por tu red
PASSWORD = 'FamiliaCoca321' 
...
UMBRAL      = 800        # Diferencia min-max para detectar voz (ajustar si hace falta)
...
def hay_voz():
    """
    Lee 200 muestras rápidas y calcula la energía (max - min).
    Si supera UMBRAL, hay voz. Ajusta UMBRAL si detecta ruido
    sin que hables, o si no detecta cuando sí hablas.
    """
    muestras = [adc.read() for _ in range(200)]
    energia = max(muestras) - min(muestras)
    return energia > UMBRAL
...
def main():
    ...
    print(f'SAMPLE_RATE: {SAMPLE_RATE} Hz | UMBRAL: {UMBRAL}')
    ...
    while True:
        ...
        try:
            while True:
                ...
                # 4. Recibir audio de respuesta de la laptop
                header = recibir_completo(conn_spk, 4)
                if not header or len(header) < 4:
                    raise OSError('Conexion del speaker cerrada por la laptop')
                tam = struct.unpack('>I', header)[0]
                print('Recibiendo respuesta:', tam, 'bytes...')
                respuesta = recibir_completo(conn_spk, tam)
                if len(respuesta) < tam:
                    raise OSError('Respuesta incompleta (conexion cortada)')

                # 5. Reproducir respuesta por el DAC → PAM8403 → speaker
                print('Reproduciendo', len(respuesta), 'bytes...')
